...
import { createRent, updateRent, deleteRent } from "../../../services/api";
import api from "../../../services/api";
import API_ENDPOINTS from "../../../services/apiEndpoints";

const LeaseForm = ({ lease = null, onSave, onCancel, isEdit = false }) => {
  const { direction } = useLanguageStore();
  const { t } = useTranslation();

  // Map lease data to form structure (same as ReservationForm)
  const [formData, setFormData] = useState({
    unit: lease?.unit || "",
    tenant: lease?.tenant || "",
    rent_start: lease?.rent_start || lease?.startDate || "",
    rent_end: lease?.rent_end || lease?.endDate || "",
    total_amount: lease?.total_amount || lease?.rent || "",
    payment_status: lease?.payment_status || "paid",
    payment_method: lease?.payment_method || "cash",
    payment_date: lease?.payment_date?.slice(0, 10) || "",
    notes: lease?.notes || "",
    attachment: null,
  });
  ...
  React.useEffect(() => {
    const fetchLists = async () => {
      try {
        ...
        const tenants = (tenantsData?.results || tenantsData || []).map(
          (t) => ({
            id: t.id,
  ...
  const validateForm = () => {
    ...
    if (formData.rent_start && formData.rent_end) {
      const startDate = new Date(formData.rent_start);
      const endDate = new Date(formData.rent_end);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let result;
      if (isEdit && lease?.id) {
        result = await updateRent(lease.id, fd);
      } else {
        result = await createRent(fd);
      }
      onSave?.(result);
    } catch (err) {
      ...
      Object.keys(apiErrors).forEach((k) => {
        mapped[k] = Array.isArray(apiErrors[k])
          ? apiErrors[k].join(" ")
          : String(apiErrors[k]);
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteRent(lease.id);
      onSave?.({ deleted: true, id: lease.id });
    } catch (err) {
      alert(err?.message || "Failed to delete rent");
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    <option value="cash">
                      {direction === "rtl" ? "نقدًا" : "Cash"}
                    </option>
                    <option value="bank_transfer">
                      {direction === "rtl" ? "تحويل بنكي" : "Bank Transfer"}
                    </option>
                    <option value="credit_card">
                      {direction === "rtl" ? "بطاقة ائتمان" : "Credit Card"}
                    </option>
                    <option value="online_payment">
                      {direction === "rtl" ? "دفع إلكتروني" : "Online Payment"}
                    </option>
                  </select>
            ...
            >
              ...
              {isEdit && lease?.id ? (
                <Button
                  ...
                  className="w-full sm:w-auto px-6 py-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                  disabled={isLoading}
                >
                  <X
                    className={`h-4 w-4 ${
